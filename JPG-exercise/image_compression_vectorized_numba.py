...
# Element-wise quantization using @vectorize
@vectorize(['int8(float64, float64)'], nopython=True)
def quantize(val, qval):
    return np.int8(np.round(val / qval))

@vectorize(['float64(int8, float64)'], nopython=True)
def dequantize(val, qval):
    return val * qval

@vectorize(['uint8(float64)'], nopython=True)
def safe_uint8(val):
    return np.uint8(min(255, max(0, round(val))))
